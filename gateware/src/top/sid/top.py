...
class SIDSoc(TiliquaSoc):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            ...
            pmod0.i_cal.payload[0].numerator().eq(sid.voice0_dca),
            pmod0.i_cal.payload[1].numerator().eq(sid.voice1_dca),
            pmod0.i_cal.payload[2].numerator().eq(sid.voice2_dca),
            pmod0.i_cal.payload[3].numerator().eq(self.sid_periph.last_audio_left>>8),
        ]
