...
class Stroke(wiring.Component):
    ...
    def elaborate(self, platform) -> Module:
        ...
        def normalize_scale(sq: fixed.Value, scale: Signal(unsigned(4))):
        ...
        with m.FSM() as fsm:
            ...
            with m.State('LATCH0'):
                ...
                with m.If(self.point_stream.valid):
                    m.d.sync += [
                        sample_x.eq(normalize_scale(self.point_stream.payload[0], self.scale_x) + self.x_offset),
                        # invert sample_y for positive scope -> up
                        sample_y.eq(normalize_scale(-self.point_stream.payload[1], self.scale_y) + self.y_offset),
                        sample_p.eq(Mux(self.scale_p != 0xf, normalize_scale(self.point_stream.payload[2], self.scale_p), 0)),
                        sample_c.eq(Mux(self.scale_c != 0xf, normalize_scale(self.point_stream.payload[3], self.scale_c), 0)),
                    ]
                    m.next = 'LATCH1'
