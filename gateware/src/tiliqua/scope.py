...
class ScopeTracePeripheral(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        def normalize_scale(data: Signal(signed(16))):
        ...
        with m.If(self._timebase.f.timebase.w_stb):
            m.d.sync += timebase.as_value().eq(normalize_scale(self._timebase.f.timebase.w_data))
        ...
        with m.If(self._trigger_lvl.f.trigger_level.w_stb):
            m.d.sync += trigger_lvl.as_value().eq(normalize_scale(self._trigger_lvl.f.trigger_level.w_data))
