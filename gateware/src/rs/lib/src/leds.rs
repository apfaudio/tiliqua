...
pub fn mobo_pca9635_set_bargraph<T: OptionsEncoderInterface>(
    optif: &T, leds: &mut [u8; 16], toggle: bool) {
    if let Some(n) = optif.selected() {
        if n > 7 {
            return;
        }

        let o = optif.current_opts()[n];
        let c = o.percent();
        for n in 0..6 {
