...
class MidiTests(unittest.TestCase):
    ...
    def test_midi_decode(self, name, is_usb):
        ...
        async def testbench(ctx):
            ...
            self.assertEqual(p.status0, midi.Status.NOTE_ON)
            self.assertEqual(p.status1.channel, 2)
            self.assertEqual(p.data1.note, 0x48)
            self.assertEqual(p.data2.velocity, 0x96)
    ...
    def test_midi_voice_tracker(self):
        ...
        async def stimulus_notes(ctx):
            ...
            for note in note_range:
                ...
                ctx.set(dut.i.valid, 1)
                ctx.set(dut.i.payload.status0, midi.Status.NOTE_ON)
                ctx.set(dut.i.payload.status1.channel, 1)
                ctx.set(dut.i.payload.data1.note, note)
                ctx.set(dut.i.payload.data2.velocity, 0x60)
            ...
            for note in note_range:
                ctx.set(dut.i.valid, 1)
                ctx.set(dut.i.payload.status0, midi.Status.NOTE_OFF)
                ctx.set(dut.i.payload.status1.channel, 1)
                ctx.set(dut.i.payload.data1.note, note)
                ctx.set(dut.i.payload.data2.velocity, 0x30)
                await ctx.tick().until(dut.i.ready)
                ctx.set(dut.i.valid, 0)
                await ctx.tick()
